...
class MultiHeadAttentionwDropout(nn.Module):
    ...
    def forward(self, 
        ...
        if self.flash_attn:
            out, attn = self.attn(query_w_pos, key_w_pos, value, key_padding_mask = key_padding_mask)
        else:
            out, attn = self.attn(query=query_w_pos, key=key_w_pos, value=value, attn_mask=attn_mask ,key_padding_mask = key_padding_mask)
...
class DREAMTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, query, key, query_pos, key_pos, attn_mask=None,key_padding_mask =None):
        """ Forward function for transformer decoder layer
        Args:
            query: shape [num_query, batch_size, embed_dims]
            key: shape [num_key, batch_size, embed_dims]
        ...
        # query = self.transformer_layers[1](query)
        # 3. Multi-head Cross-attention (between queries and keys)
        query, attn1 = self.transformer_layers[0](query, key, key, query_pos, key_pos, attn_mask, key_padding_mask)
        # 4. LayerNorm
        query = self.transformer_layers[1](query)
        # 5. Feed-forward Network
...
@TRANSFORMER.register_module()
class DREAMTransformerDecoder(nn.Module):
    def __init__(self, 
                 num_layers,
                 embed_dims,
                 num_heads,
    ...
    def forward(self, query, key, query_pos=None, key_pos=None, attn_mask=None,key_padding_mask=None):
        """ Forward function for transformer decoder
        Args:
            query: shape [num_query, batch_size, embed_dims]
            key: shape [num_key, batch_size, embed_dims]
        ...
        for layer in self._layers:
            if self._with_cp and self.training:
                query = cp.checkpoint(layer, query, key, query_pos, key_pos, attn_mask, key_padding_mask)
            else:
                query = layer(query, key, query_pos, key_pos, attn_mask, key_padding_mask)
            return_val.append(query)
