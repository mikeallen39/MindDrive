...
def load_model_and_pipeline(config_path, ckpt_path):
    ...
    for pipeline in cfg.inference_only_pipeline:
        if pipeline["type"] not in ['LoadMultiViewImageFromFilesInCeph']:
            inference_only_pipeline.append(pipeline)
    inference_only_pipeline = Compose(inference_only_pipeline)
    return model, inference_only_pipeline
...
class LeaderboardEvaluator(object):
    ...
    def _load_and_run_scenario(self, args, config, model, pipeline):
        """
        ...
        try:
            ...
            if model is not None and pipeline is not None:
                self.agent_instance.setup_model(model, pipeline)
    ...
    def run(self, args):
        ...
        if args.load_once == "1":
            print("Load model & checkpoint out of the loop.")
            config_path, ckpt_path = args.agent_config.split('+')
            model, pipeline = load_model_and_pipeline(config_path, ckpt_path)
        else:
        ...
        while route_indexer.peek() and not crashed:
            ...
            crashed = self._load_and_run_scenario(args, config, model, pipeline)
...
def main():
    ...
    parser.add_argument('--load-once', type=str, default="0")
