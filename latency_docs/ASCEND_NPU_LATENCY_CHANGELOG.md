...
## 13. `3B` offline latency 的 pipeline 错配问题
...
`scripts/benchmark_minddrive_latency_offline.py` 已改为在构建 offline dataset
时优先使用 `cfg.inference_only_pipeline`（若配置提供该字段）。

这样 `3B` offline latency 与 `0.5B` offline latency 在 prompt / image token
语义上保持一致，都只测规划生成路径，而不会误走多轮 `critical_qa` test
pipeline。
...
### 17.1 为了让 `3B` benchmark 稳定运行所做的适配
